...
class Connection:
    ...
    def _connect(self) -> None:
        ...
        try:
            ...
            raise DatabaseError(
                f"Driver module not installed for {self.driver}: {str(e)}"
            )
        ...
        try:
            with self.lock:
                ...
                while not self.pool.empty():
                    ...
                    if (
                        current_time - conn.last_used > self.idle_timeout
                        and active_count > self.min_connections
                    ):
                ...
                while active_count < self.min_connections:
                    try:
                        ...
                        logger.error(
                            f"Failed to add connection during maintenance: {str(e)}"
                        )
                        break
